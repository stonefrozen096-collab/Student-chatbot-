...
// Load preloaded commands
document.addEventListener('DOMContentLoaded', () => {
  if(masterCommands.length===0){
    preCommands.forEach(c=>masterCommands.push(c));
  }
...
// Execute command with animated overlay
...
function showCommandOverlay(name,user){
  ...
  setTimeout(()=>{
    overlay.animate([{opacity:1,transform:'scale(1)'},{opacity:0,transform:'scale(0.9)'}],{duration:400,fill:'forwards'}).onfinish=()=>overlay.remove();
  },1500);
}
...
// Prompt lock all temporary duration
...
function lockAllUsers(sec=30,permanent=false){
  ...
  if(!permanent){
    ...
    countdownInterval = setInterval(()=>updateCountdown(sec),1000);
    setTimeout(()=>{
...
// Restore last deleted test example
