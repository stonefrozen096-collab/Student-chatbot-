// server.js — Production-ready Student Assistant (FULL, updated)
// Save/replace your existing server.js with this file
...
// tiny wrapper to avoid repeating try/catch in every route
...
// ---------------- 2FA / RESET PASSWORD ----------------
// Primary route: request reset (returns debugCode if email not configured)
async function handleRequestReset(req, res) {
  ...
  // Email not configured? return debug code (dev)
...
// Primary reset handler (email + code + newPassword)
...
app.post('/admin/forgot-password', body('email').isEmail(), wrap(handleRequestReset)); // compatibility alias

app.post('/api/reset-password', wrap(handleResetPassword));
app.post('/admin/reset-password', wrap(handleResetPassword)); // alias for older frontend POST target

// Support token-in-url (optional): /api/reset-password/:token  (uses token + email/newPassword in body or query)
app.post('/api/reset-password/:token', wrap(async (req, res) => {
  // token here is the numeric code we generated previously (10-min code)
  ...
  // reuse handler behavior
  req.body = { email, code: token, newPassword };
  return handleResetPassword(req, res);
}));

// Serve admin static pages explicitly (so visiting /admin/forgot-password works)
